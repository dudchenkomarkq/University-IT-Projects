...
## 🛠 Tech Stack & Skills
* **Operating Systems:** Linux (RHEL, Ubuntu, Debian), Windows Server 2019.
...
* **Security:** **SSH (Secure access/Key auth)**, Fail2Ban (WIP).
...
* **Partitioning:** Working with `fdisk`, `gdisk`, and `blkid`.
* **RAID:** Implementing software RAID 0, 1, and 5 using `mdadm`.
* **LVM:** Managing Logical Volume Management (Physical Volumes, Volume Groups, Logical Volumes).
* **Mounting:** Configuring persistent storage via `/etc/fstab`.
...
* [Storage & LVM Management](Montowanie%20zasobów%20lokalnychh.pdf) - Linux disk subsystem configuration.
